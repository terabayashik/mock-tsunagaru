...
export async function createTestCSV(filename: string = 'test-data.csv'): Promise<string> {
...
export async function setupTestData(page: Page) {
  ...
  await page.locator('[class*="mantine-Notification-title"]').filter({ hasText: /テストデータ作成完了|一部のテストデータ作成に失敗/ }).first().waitFor({ timeout: 30000 });
