...
export class BasePage {
  ...
  // 共通のモーダル操作
  async closeModal() {
    const closeButton = this.page.getByRole('button').filter({ has: this.page.locator('svg') }).first();
  ...
  async waitForSuccessNotification() {
    ...
    await this.page.locator('[class*="mantine-Notification-title"]').filter({ hasText: /成功|完了|失敗/ }).first().waitFor({ timeout: 30000 });
  }
